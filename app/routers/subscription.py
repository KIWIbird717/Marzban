...
from config import (
    ...
    SUB_UPDATE_INTERVAL,
    SUB_ANNOUNCE,
    SUB_HIDE_SETTINGS_HAPP,
    SUB_SUBSCRIPTION_BASE_URL,
...
def build_url(base, *paths):
    return "/".join([base.rstrip("/")] + [p.strip("/") for p in paths])


def modify_profile_web_page_url(token: str, request: Request):
    """
    Modifies `profile-web-page-url` depending on configuration
    """
    if not SUB_SUBSCRIPTION_BASE_URL:
...
def user_subscription(
    ...
    response_headers = {
        ...
        "profile-web-page-url": modify_profile_web_page_url(token, request),
    ...
    if re.match(r'^([Cc]lash-verge|[Cc]lash[-\.]?[Mm]eta|[Ff][Ll][Cc]lash|[Mm]ihomo)', user_agent):
        ...
        version_str = re.match(
            r'^v2rayNG/(\d+\.\d+\.\d+)', user_agent).group(1)
